...
class _PersonalDetailsScreenState extends State<PersonalDetailsScreen> {
  ...
  Future<void> _loadPersonalDetails() async {
    ...
    try {
      ...
      if (activeCvId != null) {
        final prefs = await SharedPreferences.getInstance();
        await prefs.setString('active_cv_id', activeCvId);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data.isNotEmpty) {
          ...
          _firstNameController.text = remoteData['firstName'] ?? "";
          _lastNameController.text = remoteData['lastName'] ?? "";
          _emailController.text = remoteData['email'] ?? "";
          _phoneController.text = remoteData['phone'] ?? "";
          _addressController.text = remoteData['address'] ?? "";
          _cityStateController.text = remoteData['cityState'] ?? "";
          _countryController.text = remoteData['country'] ?? "";
          _dateOfBirthController.text = remoteData['dateOfBirth'] ?? "";
          _placeOfBirthController.text = remoteData['placeOfBirth'] ?? "";
          _genderController.text = remoteData['gender'] ?? "";
          _nationalityController.text = remoteData['nationality'] ?? "";
          _linkedinController.text = remoteData['linkedin'] ?? "";
          _githubController.text = remoteData['github'] ?? "";
  ...
  Future<void> _savePersonalDetails() async {
    ...
    try {
      ...
      if (widget.cvId != null) {
        ...
        // Try getting CV ID from shared prefs if null in widget
        final prefs = await SharedPreferences.getInstance();
        final activeCvId = prefs.getString('active_cv_id');
        if (activeCvId != null) {
          await _saveRemoteDetails();
        } else {
          debugPrint('⚠️ [SYNC] Skipping remote sync: No CV ID provided.');
        }
      }
    } catch (e) {
  ...
  Future<void> _saveRemoteDetails() async {
    ...
    try {
      ...
      debugPrint('🆕 [SYNC] Remote record synced via POST');
    } catch (e) {
