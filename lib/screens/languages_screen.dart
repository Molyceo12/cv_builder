...
class LanguageEntry {
  ...
  void Function(LanguageEntry)? onUpdate;
...
class _LanguagesScreenState extends State<LanguagesScreen> {
  ...
  Future<bool> _saveData({LanguageEntry? targetEntry}) async {
    final prefs = await SharedPreferences.getInstance();
    final activeCvId = prefs.getString('active_cv_id');
    ...
    try {
      final listToSync = targetEntry != null 
          ? [targetEntry] 
          : _languagesList.where((e) => e.nameController.text.isNotEmpty).toList();
      ...
      final response = await _apiClient.dio.post('api/language/', data: bulkData);
        ...
        for (var processed in processedList) {
          ...
          final entry = listToSync.firstWhere(
            (e) => (e.remoteId != null && e.remoteId == processed['id']?.toString()) ||
                   (e.nameController.text == processed['language']),
      ...
      debugPrint('Error syncing languages: $e');
      return false;
    } finally {
  ...
  void _triggerAutoSave(LanguageEntry entry) {
  ...
  void _addSuggestedLanguage(String name) {
    ...
    LanguageEntry? target;
    setState(() {
      if (emptyIndex != -1) {
        target = _languagesList[emptyIndex];
        target!.nameController.text = name;
        target!.isExpanded = true;
      } else {
        target = LanguageEntry(onUpdate: _triggerAutoSave);
        target!.nameController.text = name;
        target!.isExpanded = true;
        _languagesList.add(target!);
      }
    });
            ...
            child: TextButton(
              onPressed: () async {
                ...
                messenger.showSnackBar(
                  SnackBar(
                    content: Text(success ? 'Languages saved successfully!' : 'Error saving languages'),
