...
class _EducationHistoryScreenState extends State<EducationHistoryScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
          ...
          setState(() {
            ...
            if (data.isNotEmpty) {
              for (var edu in data) {
                ...
                entry.remoteId = edu['id']?.toString();
                entry.schoolController.text = edu['school'] ?? '';
                entry.degreeController.text = edu['degree'] ?? '';
                entry.startDateController.text = edu['start_date'] ?? '';
                entry.isCurrent = edu['is_current'] ?? false;
                if (entry.isCurrent) {
                  entry.endDateController.text = 'Present';
                } else {
                  entry.endDateController.text = edu['end_date'] ?? '';
                }
                entry.cityStateController.text = edu['city_state'] ?? '';
      ...
      debugPrint('Error loading education via API: $e');
      _addEmptyEducation();
    }
  ...
  void _setupAutoSave() {
    for (var entry in _educationList) {
      _addListenersToEntry(entry);
    }
  }

  void _addListenersToEntry(EducationEntry entry) {
    void autoSaveWrapper() => _autoSave();
    entry.schoolController.addListener(autoSaveWrapper);
    entry.degreeController.addListener(autoSaveWrapper);
  ...
  Future<bool> _saveData() async {
    ...
    try {
      for (var entry in _educationList) {
        if (entry.schoolController.text.isEmpty) continue;

        final data = {
          "cv": activeCvId,
          "school": entry.schoolController.text,
          "degree": entry.degreeController.text,
          "start_date": entry.startDateController.text,
          "end_date": entry.isCurrent ? "Present" : entry.endDateController.text,
        ...
        if (entry.remoteId != null) {
          ...
          if (response.statusCode == 201 || response.statusCode == 200) {
            ...
            if (respData is Map && respData['body'] != null) {
              entry.remoteId = respData['body']['id']?.toString();
            } else if (respData is Map && respData['id'] != null) {
              entry.remoteId = respData['id']?.toString();
            }
          }
        }
      ...
      debugPrint('Error syncing education: $e');
      return false;
    } finally {
