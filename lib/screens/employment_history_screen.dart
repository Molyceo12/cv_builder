...
class WorkExperienceEntry {
  String? remoteId; // Track server-side ID
...
class _EmploymentHistoryScreenState extends State<EmploymentHistoryScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
          ...
          if (data.isNotEmpty) {
            setState(() {
              ...
              for (var exp in data) {
                ...
                entry.remoteId = exp['id']?.toString();
                entry.jobTitleController.text = exp['job_title'] ?? '';
                entry.employerController.text = exp['employer'] ?? '';
                
                // Convert YYYY-MM-DD back to MM / YYYY for UI
                ...
                entry.startDateController.text = formatUiDate(exp['start_date']);
                entry.isCurrent = exp['is_current'] ?? false;
                if (entry.isCurrent) {
                  entry.endDateController.text = 'Present';
                } else {
                  entry.endDateController.text = formatUiDate(exp['end_date']);
                }
                
                entry.cityStateController.text = exp['city_state'] ?? '';
                
                // Description can be a list or a string
                if (exp['description'] is List) {
                  entry.descriptionController.text = (exp['description'] as List)
                      .map((item) => "• $item")
                      .join('\n');
                } else {
      ...
      debugPrint('Error loading remote work experience: $e');
      _addEmptyExperience();
    }
  ...
  void _setupAutoSave() {
    for (var entry in _experiences) {
      _addListenersToEntry(entry);
    }
  }

  void _addListenersToEntry(WorkExperienceEntry entry) {
    void autoSaveWrapper() => _autoSave();
    entry.jobTitleController.addListener(autoSaveWrapper);
    entry.employerController.addListener(autoSaveWrapper);
    entry.startDateController.addListener(autoSaveWrapper);
    entry.endDateController.addListener(autoSaveWrapper);
    entry.cityStateController.addListener(autoSaveWrapper);
    entry.descriptionController.addListener(autoSaveWrapper);
  }
  ...
  void _addEmptyExperience() {
    setState(() {
      final newEntry = WorkExperienceEntry();
      _experiences.add(newEntry);
      _addListenersToEntry(newEntry);
    });
  }
  ...
  Future<bool> _saveData() async {
    ...
    if (activeCvId != null) {
      return await _saveRemoteExperiences(activeCvId);
    } else {
      debugPrint('⚠️ [SAVE] Cannot save: No active CV ID found.');
      return false;
    }
  }
      ...
      for (var entry in _experiences) {
        ...
        if (entry.remoteId != null) {
          ...
          response = await _apiClient.dio.patch(
            'api/work-experiences/${entry.remoteId}/',
            data: data,
          );
        } else {
          // CREATE new
          ...
          if (response.statusCode == 201 || response.statusCode == 200) {
             // Save the new ID returned by server
             final responseData = response.data;
             if (responseData is Map && responseData['body'] != null) {
               entry.remoteId = responseData['body']['id']?.toString();
             } else if (responseData is Map && responseData['id'] != null) {
               entry.remoteId = responseData['id']?.toString();
             }
          }
        }
