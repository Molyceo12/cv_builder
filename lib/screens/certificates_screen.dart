...
class CertificateEntry {
  ...
  CertificateEntry({this.remoteId});
  ...
  Map<String, dynamic> toPayload(String cvId) {
    return {
      ...
      'description': descriptionController.text.replaceAll('•', '').trim(),
    };
  }
}
...
class _CertificatesScreenState extends State<CertificatesScreen> {
    ...
    try {
      ...
      if (activeCvId != null) {
        ...
        if (response.statusCode == 200) {
          final List<dynamic> certs = response.data;
          setState(() {
            _certificatesList.clear();
            if (certs.isNotEmpty) {
              for (var c in certs) {
                final entry = CertificateEntry(remoteId: c['id']);
                entry.titleController.text = c['title'] ?? '';
                entry.dateController.text = c['date'] ?? '';
                ...
                if (desc is List) {
                  ...
                  entry.descriptionController.text = desc.replaceAll('•', '').trim();
                }
  ...
  void _addListeners(CertificateEntry entry) {
    entry.titleController.addListener(_triggerAutoSave);
    entry.dateController.addListener(_triggerAutoSave);
  ...
  Future<bool> _saveData({bool force = false}) async {
    ...
    try {
      for (var task in tasks) {
        final entry = task['entry'] as CertificateEntry;
        ...
        try {
          ...
          if (entry.remoteId != null) {
            response = await _apiClient.dio.patch(
              'api/certificate/${entry.remoteId}/',
              data: payload,
            );
          } else {
          ...
          if (response.statusCode == 200 || response.statusCode == 201) {
            ...
            entry.lastSavedSnapshot = task['snapshot'];
          } else {
            allSuccess = false;
          }
        } catch (e) {
          debugPrint("Error saving individual certificate: $e");
          allSuccess = false;
        }
      }
    } catch (e) {
      debugPrint("Error in certificate save loop: $e");
      allSuccess = false;
    } finally {
